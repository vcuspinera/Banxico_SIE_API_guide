...
To show the usage of the SIE API with `siebanxicor` R-package, we will run through an example using the time series of _Annual counterfeit domestic banknotes detected_ (serie SM1255) and _Annual counterfeit domestic banknotes detected_ (SM1266).

The first step, after installed package, is to load the library.
```{r}
library("siebanxicor")
```
